...
void compCases() {
    ...
    for(int i = 0; i < _N_PAIRS_; i ++) {
        for(int j = 0; j < _N_CASES_; j++) {
            fileIn[i][j] = new TFile(Form("./outputCorrFunc/%s%s.root",cases[j].Data(),pairs[i].Data()));
            hist[i][j] = (TH1D*) fileIn[i][j]->Get(Form("%sE0%sCorrFunc",cases[j].Data(),pairs[i].Data()));
    ...
    TFile *fileOut = new TFile("./outputCompCases/compCases.root", "RECREATE");
    TCanvas *can[_N_PAIRS_];
    for(int i = 0; i < _N_PAIRS_; i++) {
            
        for(int j = 0; j < _N_CASES_; j++) {
            hist[i][j]->GetFunction("fun")->SetLineColor(colors[j+1]);
            hist[i][j]->GetXaxis()->SetRangeUser(0.0,0.15);
        }


                        
        can[i] = new TCanvas(Form("%sCasesComp",pairs[i].Data()),Form("%sCasesComp",pairs[i].Data()),800,800);
        if(i==0) {
            multiplePlot(hist[i],can[i],casesNice,3,false);
            makePaveText(can[i],pairsNice[i].Data(),0.55,0.3,0.7,0.35);
            makePaveText(can[i],epsilonNice[0].Data(),0.55,0.35,0.7,0.4);
        }
        else {
            multiplePlot(hist[i],can[i],casesNice,3,true);
            makePaveText(can[i],pairsNice[i].Data(),0.55,0.7,0.7,0.75);
            makePaveText(can[i],epsilonNice[0].Data(),0.55,0.75,0.7,0.8);
        }

        fileOut->cd();
        can[i]->Write();
        can[i]->SaveAs(Form("./ladne/compCases/%sCasesComp.pdf",pairs[i].Data()));
