...
void multiplePlot(TH1D *histTab[], TCanvas* can, TString* entry, size_t n) {
    ...
    TLegend* legend	= new TLegend(0.63,0.2,0.93,0.5, "",	"NDC");
...
void fixedPairs() {
    ...
    for(int i = 0; i < _N_CASES_; i ++) {
        for(int j = 0; j < _N_PAIRS_; j++) {
            ...
            multiplePlot(hist[i][j][0],can[i][j][0],entries,7);
            makePaveText(can[i][j][0],casesNice[i].Data(),0.6,0.7,0.8,0.75);
            makePaveText(can[i][j][0],pairsNice[i].Data(),0.6,0.75,0.8,0.8);
            fileOut[j]->cd();
            can[i][j][0]->Write();
            ...
            if (j == 0) {
                ...
                multiplePlot(hist[i][j][1],can[i][j][1],entries,7);
                makePaveText(can[i][j][1],casesNice[i].Data(),0.6,0.7,0.8,0.75);
                makePaveText(can[i][j][1],pairsNice[i].Data(),0.6,0.75,0.8,0.8);
                fileOut[j]->cd();
                can[i][j][1]->Write();
            }
